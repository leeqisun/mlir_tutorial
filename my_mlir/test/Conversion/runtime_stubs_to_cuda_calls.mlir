// RUN: ns-opt %s --convert-north-satr-to-linalg --outline-north-star-device-kernels --lower-north-star-device-kernels-to-loops --lower-north-star-host-to-npu-calls --lower-north-star-npu-calls-to-runtime --lower-north-star-runtime-stubs-to-cuda-calls --split-input-file | FileCheck %s
module @NorthStar {
  // CHECK-DAG: func.func private @north_star_cuda_launch_f32(i64, tensor<*xf32>) -> tensor<*xf32> attributes {north_star.runtime_cuda_c_api, north_star.target = "nvidia_rtx_5060_cuda"}
  // CHECK-DAG: func.func private @north_star_cuda_launch_softmax_2_d_d_softmax_2_d_d__device_1(
  // CHECK-DAG: device_id = 1 : i64
  // CHECK-DAG: north_star.descriptor_id = 0 : i64
  // CHECK-DAG: north_star.kernel_name = "softmax_2_d_d_softmax_2_d_d__device_1"
  // CHECK-DAG: north_star.runtime_cuda_bridge
  ...
  // CHECK-DAG: %[[RAW0:.+]] = call @north_star_cuda_launch_f32(%[[DESC0]], %[[ERASE0]]) : (i64, tensor<*xf32>) -> tensor<*xf32>
  // CHECK-DAG: %[[RANK0:.+]] = tensor.cast %[[RAW0]] : tensor<*xf32> to tensor<2x?x?xf32>
  // CHECK-DAG: return %[[RANK0]] : tensor<2x?x?xf32>
  // CHECK-DAG: func.func private @north_star_cuda_launch_softmax_2_d_d_softmax_2_d_d__device_2(
  // CHECK-DAG: device_id = 2 : i64
  // CHECK-DAG: north_star.descriptor_id = 1 : i64
  ...
  // CHECK-DAG: %[[RAW1:.+]] = call @north_star_cuda_launch_f32(%[[DESC1]], %[[ERASE1]]) : (i64, tensor<*xf32>) -> tensor<*xf32>
  ...
  // CHECK-DAG: north_star.runtime_stub
  // CHECK-DAG: north_star.descriptor_id = 0 : i64
  // CHECK-DAG: north_star.kernel_name = "softmax_2_d_d_softmax_2_d_d__device_1"
  // CHECK-DAG: north_star.target = "nvidia_rtx_5060_cuda"
  // CHECK-DAG: call @north_star_cuda_launch_softmax_2_d_d_softmax_2_d_d__device_1
  // CHECK-DAG: func.func private @north_star_runtime_launch_softmax_2_d_d_softmax_2_d_d__device_2
  // CHECK-DAG: north_star.runtime_stub
  // CHECK-DAG: north_star.descriptor_id = 1 : i64
  // CHECK-DAG: north_star.kernel_name = "softmax_2_d_d_softmax_2_d_d__device_2"
  // CHECK-DAG: north_star.target = "nvidia_rtx_5060_cuda"
  // CHECK-DAG: call @north_star_cuda_launch_softmax_2_d_d_softmax_2_d_d__device_2
