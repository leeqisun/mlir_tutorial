// RUN: ns-opt %s --convert-north-satr-to-linalg --outline-north-star-device-kernels --lower-north-star-device-kernels-to-loops --lower-north-star-host-to-npu-calls --split-input-file | FileCheck %s
module @NorthStar {
  // CHECK: func.func @main
  // CHECK: north_star.npu_call @softmax_2_d_d_softmax_2_d_d__device_1 target = "amd_h350_npu" device = 1
  // CHECK: north_star.npu_call @softmax_2_d_d_softmax_2_d_d__device_2 target = "amd_h350_npu" device = 2
  // CHECK-NOT: call @softmax_2_d_d_softmax_2_d_d__device_1
  // CHECK: func.func private @softmax_2_d_d_softmax_2_d_d__device_1
  // CHECK-SAME: north_star.kernel
  // CHECK-SAME: north_star.target = "amd_h350_npu"
  // CHECK: scf.for
  // CHECK: math.exp
