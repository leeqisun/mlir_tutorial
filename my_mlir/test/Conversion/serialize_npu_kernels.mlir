// RUN: ns-opt %s --convert-north-satr-to-linalg --outline-north-star-device-kernels --lower-north-star-device-kernels-to-loops --lower-north-star-host-to-npu-calls --serialize-north-star-npu-kernels --split-input-file | FileCheck %s
module @NorthStar {
  // CHECK: module @NorthStar attributes {north_star.serialized_kernels = [
  // CHECK-SAME: {device_id = 1 : i64
  // CHECK-SAME: name = "softmax_2_d_d_softmax_2_d_d__device_1"
  // CHECK-SAME: outputs = [{element_type = f32
  // CHECK-SAME: type = tensor<2x?x?xf32>}]
  // CHECK-SAME: target = "amd_h350_npu"
  // CHECK-SAME: {device_id = 2 : i64
  // CHECK-SAME: name = "softmax_2_d_d_softmax_2_d_d__device_2"
  // CHECK-SAME: target = "amd_h350_npu"
