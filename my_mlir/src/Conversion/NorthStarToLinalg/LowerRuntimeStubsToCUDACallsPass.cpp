...
#define GEN_PASS_DEF_LOWERNORTHSTARRUNTIMESTUBSTOCUDACALLSPASS
#include "Conversion/Passes.h.inc"

namespace {

struct LowerNorthStarRuntimeStubsToCUDACallsPass
    : public impl::LowerNorthStarRuntimeStubsToCUDACallsPassBase<
          LowerNorthStarRuntimeStubsToCUDACallsPass> {
...
static func::FuncOp getOrCreateBridgeDecl(func::FuncOp runtimeStub,
  ...
  std::string bridgeName = getCUDARuntimeBridgeName(kernelName.getValue());
  ...
  bridge.setPrivate();
  bridge->setAttr(KNPURuntimeCUDABridgeAttr, builder.getUnitAttr());
  ...
  bridge->setAttr(KNPUTargetAttr, builder.getStringAttr(KRTX5060CUDATarget));
...
static func::FuncOp getOrCreateUnifiedLaunchApi(ModuleOp module,
                                                SymbolTable &symbolTable) {
  auto apiName = getCUDAUnifiedLaunchAPIName();
  ...
  api.setPrivate();
  api->setAttr(KNPURuntimeCUDACApiAttr, builder.getUnitAttr());
  api->setAttr(KNPUTargetAttr, builder.getStringAttr(KRTX5060CUDATarget));
...
void LowerNorthStarRuntimeStubsToCUDACallsPass::runOnOperation() {
  ...
  for (auto [descriptorId, runtimeStub] : llvm::enumerate(runtimeStubs)) {
    ...
    runtimeStub->setAttr(KNPURuntimeDescriptorIdAttr,
                         IntegerAttr::get(IntegerType::get(module.getContext(), 64),
                                          descriptorId));
    runtimeStub->setAttr(KNPUTargetAttr,
                         StringAttr::get(module.getContext(), KRTX5060CUDATarget));
    auto bridge =
