...
::llvm::LogicalResult NPUCallOp::verify() {
  auto callee = SymbolTable::lookupNearestSymbolFrom<func::FuncOp>(
      getOperation(), getCalleeAttr());
  if (!callee)
    return emitOpError("requires a valid func.func callee");
  if (!callee->hasAttr("north_star.kernel"))
    return emitOpError("callee must be an outlined north_star kernel");
  if (callee.getNumArguments() != getInputs().size())
    return emitOpError("input count must match kernel arguments");
  if (callee.getNumResults() != getNumResults())
    return emitOpError("result count must match kernel results");
  return llvm::success();
}
      ...
      if (auto shaped = llvm::dyn_cast_or_null<ShapedType>(type)) {
        for (auto index : llvm::index_range(0, shaped.getRank())) {
          if (shaped.isDynamicDim(index)) {
            ...
            name.append(std::to_string(shaped.getDimSize(index)));
            name.append("_");
          }
        }
      }
