...
void CH5() {
  ...
  auto module = mlir::ModuleOp::create(loc, "NorthStar");
  builder.setInsertionPointToStart(module.getBody());

  auto f32 = mlir::Float32Type::get(&context);
  auto shape = mlir::SmallVector<int64_t>({2, 2});

  auto tensor_type_1 = mlir::north_star::NSTensorType::get(&context, shape, f32, 0);
  auto tensor_type_2 = mlir::north_star::NSTensorType::get(&context, shape, f32, 1);
  auto ranked_tensor_type = mlir::RankedTensorType::get(shape, f32);

  auto const_attr_1 = mlir::DenseElementsAttr::get(ranked_tensor_type, builder.getF32FloatAttr(1.0f));
  auto const_attr_2 = mlir::DenseElementsAttr::get(ranked_tensor_type, builder.getF32FloatAttr(2.0f));

  auto const_1 = mlir::north_star::ConstOp::create(builder, loc, tensor_type_1, const_attr_1);
  auto const_2 = mlir::north_star::ConstOp::create(builder, loc, tensor_type_1, const_attr_1);
  auto const_3 = mlir::north_star::ConstOp::create(builder, loc, tensor_type_2, const_attr_2);
  auto const_4 = mlir::north_star::ConstOp::create(builder, loc, tensor_type_2, const_attr_2);
  ...
  auto buffer_op = mlir::north_star::BufferOp::create(builder, loc, mlir::ValueRange({const_1, const_3}));

  llvm::outs() << "Buffer op \n";
  buffer_op->dump();

  auto get_tensor_op_1 = mlir::north_star::GetTensorOp::create(builder, loc, tensor_type_1, buffer_op, 0);
  auto get_tensor_op_2 = mlir::north_star::GetTensorOp::create(builder, loc, tensor_type_1, buffer_op, 1);
  llvm::outs() << "Get Tensor Op : \n";
  get_tensor_op_1->dump();
  get_tensor_op_2->dump();

  auto softmax_op = mlir::north_star::SoftmaxOp::create(builder, loc, get_tensor_op_1, 1);
  llvm::outs() << "Softmax Op : \n";
  softmax_op->dump();
  auto exp_op = mlir::north_star::ExpOp::create(builder, loc, get_tensor_op_2);
  llvm::outs() << "Exp Op : \n";
  exp_op->dump();

  auto out_buffer_op = mlir::north_star::BufferOp::create(builder, loc, mlir::ValueRange({const_2, const_4}));
  auto all_to_all = mlir::north_star::AllToAllOp::create(builder, loc, buffer_op, out_buffer_op);
  llvm::outs() << "All to all : \n";
  all_to_all->dump();
}
...
void CH8() {  // 初始化方言注册器
  ...
  mlir::north_star::MarkDistributeParallelParametersPassOptions
      mark_distribute_parallel_option{.DPNums = 4, .TPNums = 1};
